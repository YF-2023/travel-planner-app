...
def budget_calc(org, dest, days, date:list , people_number=None, local_constraint = None):
    ...
    flight_price = estimate_budget(flight_data["LowestPrice"].tolist(), mode="average") * multipliers[days]["flight"]
    hotel_price = estimate_budget(hotel_data, mode="average") * multipliers[days]["hotel"]
    restaurant_price = estimate_budget(restaurant_data, mode="average") * multipliers[days]["restaurant"]

    return flight_price + hotel_price + restaurant_price
